...
# ── Feature extraction (identique au notebook) ───────────────────────────────
...
@st.cache_resource
def load_model():
    pipeline = joblib.load("models/svm_olive_pipeline.pkl")
...
CLASS_INFO = {
    "Healthy": {
        "fr": "Feuille Saine",
        "badge": "healthy",
        "desc": "La feuille ne présente aucun signe de maladie. "
    ...
    "aculus_olearius": {
        "fr": "Aculus Olearius",
        "badge": "disease",
        "desc": "Infestation par un acarien microscopique (Aculus olearius) "
                "causant des déformations foliaires et une réduction du rendement.",
        "action": "Traitement acaricide recommandé. Consultez un agronome.",
        "icon": "🔴"
    },
    "olive_peacock_spot": {
        "fr": "Tache Paon (Cycloconium)",
        "badge": "disease",
        "desc": "Maladie fongique causée par Spilocaea oleagina, "
                "reconnaissable à ses taches circulaires brun-verdâtres.",
...
<div style="display:flex; gap:1rem">
    <div class="metric" style="background:#152015; border-radius:10px; padding:0.8rem 1.2rem; flex:1; text-align:center">
        <div class="metric-val" style="font-family:'Playfair Display',serif; font-size:1.4rem; color:#a8d5a2">78.68%</div>
        <div class="metric-lbl" style="font-size:0.75rem; color:#5a7a5a; margin-top:0.2rem">Accuracy (Test)</div>
    </div>
    <div class="metric" style="background:#152015; border-radius:10px; padding:0.8rem 1.2rem; flex:1; text-align:center">
        <div class="metric-val" style="font-family:'Playfair Display',serif; font-size:1.4rem; color:#a8d5a2">SVM</div>
        <div class="metric-lbl" style="font-size:0.75rem; color:#5a7a5a; margin-top:0.2rem">Algorithme (kernel RBF)</div>
    </div>
    <div class="metric" style="background:#152015; border-radius:10px; padding:0.8rem 1.2rem; flex:1; text-align:center">
        <div class="metric-val" style="font-family:'Playfair Display',serif; font-size:1.4rem; color:#a8d5a2">3</div>
        <div class="metric-lbl" style="font-size:0.75rem; color:#5a7a5a; margin-top:0.2rem">Classes détectées</div>
    </div>
</div>
""", unsafe_allow_html=True)
